...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes('transfer(address,uint256)')));//transfer方法通过bytes打包 ，keccak256编译成哈希值，再通过bytes4打包生成16进制4位数的值
    ...
    // called once by the factory at time of deployment 初始化
    ...
    function _update(uint balance0, uint balance1, uint112 _reserve0, uint112 _reserve1) private {
        ...
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // overflow is desired 计算时间流逝
    ...
    function mint(address to) external lock returns (uint liquidity) {
        ...
        uint balance0 = IERC20(token0).balanceOf(address(this));//当前合约地址在token0代币中的余额
